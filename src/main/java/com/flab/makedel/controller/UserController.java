...
객체지향의 원칙인 단일책임 원칙을 지켜 서비스는 비즈니스 로직을 처리하게 하고 컨트롤러는 http 요청에 따라 사용할 서비스를 선택하고 매퍼(dao)에서는 sql 쿼리를 분리시켜 각각의 책임을 가지게 하는 것입니다.

또한 객체지향의 원칙인 개방폐쇄 원칙에 따라 서비스, 컨트롤러, 매퍼가 서로에 종속되지 않고 스프링 빈을 통해 객체를 주입받아 객체들끼리 서로 종속되는 일이 없도록 하기 위해서입니다.
스프링 프레임워크에서 스프링 빈은 객체의 라이프 사이클을 관리하기 때문에 서비스 컨트롤러 매퍼 각각에서 객체를 생성하거나 다른객체에 의존하지 않고 스프링 프레임워크에 의해서 객체의 라이프사이클을 관리받아야 합니다. 이러한 원칙들을 지키려면 컨트롤러 서비스 매퍼가 각각의 책임과 할일이 분명하게 나눠져야 합니다.

컨트롤러에서 비즈니스 로직을 처리하기 위해 서비스를 불러오고 서비스에서 필요한 매퍼를 호출하여 디비에 접근하는 식의 계층이 분리된 모델을 사용한다면 중복된 코드도 제거할 수 있습니다.

컨트롤러에서 여러 서비스들을 재사용하기 용이하다.
컨트롤러에서는 서비스 객체들을 주입받아 사용합니다. 비즈니스 로직이 데이터를 저장하고 수정하고 없애는 동작들을 여러 컨트롤러에서 사용할 가능성이 있으니까 컨트롤러 내에서 처리하지 않고 서비스 객체들에게 위임해서 처리하고 컨트롤러는 처리된 결과를 받아 응답하는 것입니다.
